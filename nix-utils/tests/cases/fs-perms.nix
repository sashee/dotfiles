...
in
{
  testScript = ''
    ...
    run_user("nvim-debug -c '${coreutils}/bin/install -d ~/.cache/nvim && ${coreutils}/bin/echo hi > ~/.cache/nvim/probe'")
    host = machine.succeed("cat /home/tester/.cache/nvim/probe").strip()
    assert host == "hi", f"writes to the rw-bound ~/.cache/nvim must persist on the host; got {host!r}"

    # files ro-bind: present + readable...
    content = run_user("nvim-debug -c '${coreutils}/bin/cat ~/eslint.config.js'")
    ...
    rc = run_user("nvim-debug -c '${coreutils}/bin/echo x > ~/eslint.config.js; echo RC=$?' 2>&1")
    assert "RC=0" not in rc, f"writing to the ro files-bind must fail (read-only); got {rc!r}"
    after = run_user("nvim-debug -c '${coreutils}/bin/cat ~/eslint.config.js'")
