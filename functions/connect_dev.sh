...
while true ; do
	...
	echo "Connecting" 
	ssh -q -o ConnectTimeout=10 -o "StrictHostKeyChecking no" -L 0.0.0.0:8080:localhost:8080 -L 0.0.0.0:3000:localhost:3000 -L 0.0.0.0:3001:localhost:3001 -L 0.0.0.0:8443:localhost:443 -L 0.0.0.0:35729:localhost:35729 -L 0.0.0.0:9229:localhost:9229 -L 0.0.0.0:5901:localhost:5901 -L 0.0.0.0:4200:localhost:4200 -L 0.0.0.0:4001:localhost:4001 -CtA $1@$IP AWSKEYS="$AWSKEYS" TZ=$(cat /etc/timezone) tmux new-session -A -s main
	clear;
